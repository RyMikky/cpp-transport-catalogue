...
namespace transport_catalogue {
	...
	struct Stop
	{
		...
		// ---------------------------- блок сеттеров структуры остановка -----------------------------------------

		Stop& SetStopName(std::string_view);                                // задать название остановки
		Stop& SetStopCoordinates(double, double);                           // задать координаты по двум переменным
		Stop& SetStopCoordinates(const geo::Coordinates&);                  // скопировать координаты остановки
		Stop& SetStopCoordinates(geo::Coordinates&&);                       // присвоить координаты остановки

		// ---------------------------- блок геттеров структуры остановка -----------------------------------------

		const std::string& GetStopName() const ;                            // получить название остановки
		std::string GetStopName();                                          // получить название остановки
		const geo::Coordinates& GetStopCoordinates() const;                 // задать координаты по двум переменным
		geo::Coordinates GetStopCoordinates();                              // задать координаты по двум переменным

		// ---------------------------- основные поля структуры остановка -----------------------------------------

		std::string _name;                                                  // название остановки
		geo::Coordinates _coords{ 0L,0L };                                  // координаты остановки
	...
	struct Route {
		...
		// ---------------------------- блок сеттеров структуры маршрут -------------------------------------------

		Route& SetRouteName(std::string_view);                              // задать название маршрута
		Route& SetStopsPtrs(std::vector<StopPtr>&&);                        // задать вектор указателей остановок
		Route& SetUniqueStopsQty(size_t);                                   // задать количество уникальных остановок
		Route& SetGeoRouteLength(double);                                   // задать геометрическое расстояние маршрута
		Route& SetRealRouteLength(size_t);                                  // задать реальное расстояние маршрута
		Route& SetCurvature(double);                                        // задать отклонение расстояний маршрута
		Route& SetIsCircular(bool);                                         // задать флаг кольцевого маршрута

		// ---------------------------- блок геттеров структуры маршрут -------------------------------------------

		const std::string& GetRouteName() const;                            // получить название маршрута
		const std::vector<StopPtr>& GetStopsPtrs() const;                   // получить вектор указателей остановок
		size_t GetUniqueStopsQty() const;                                   // получить количество уникальных остановок
		double GetGeoRouteLength() const;                                   // получить геометрическое расстояние маршрута
		size_t GetRealRouteLength() const;                                  // получить реальное расстояние маршрута
		double GetCurvature() const;                                        // получить отклонение расстояний маршрута
		bool GetIsCircular() const;                                         // получить флаг кольцевого маршрута

		// ---------------------------- блок основных полей маршрута ----------------------------------------------
		...
		bool _is_circular = false;                                          // флаг типа маршрута
