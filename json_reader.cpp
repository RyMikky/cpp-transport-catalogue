...
namespace transport_catalogue {
    ...
    namespace json_reader {
        ...
        // очистить базу принтера результатов
        ...
        // запуск работы в режиме ожидания и вызова тестов
        JsonReader& JsonReader::test_mode() {
            std::cerr << "Test mode is wait to execution command"sv << std::endl;
            return *this;
        }

        // запуск работы в режиме обработки одного блока. Спринт 12. Итоговое
        ...
        // запуск работы в режиме обработки make_base. Спринт 14. Итоговое 3.
        ...
        // запуск работы в режиме обработки make_base без роутера. Спринт 14. Итоговое 1.
        ...
        // запуск работы в режиме обработки process_requests. Спринт 14. Итоговое 1.
        ...
        // сбросить состояние всех компонентов программы, применяется для блока тестирования
        JsonReader& JsonReader::reset_all_modules() {
            _json_dicts.clear();                       // очищаем историю полученных JSON-блоков
            _json_requests_history.clear();            // очищаем история поступивших запросов
            _current_json_requests.clear();            // очищаем очередь текущих запростов
            _printer.ClearPrinterBase();               // очищаем базу принтера
            _handler.SetModulesToDefault();            // обнуляем обработчик и каталог
            return *this;
        }
        ...
        // парсинг имени файла для сериализации
        ...
        JsonReader& JsonReader::BaseRequestProcessor(const json::Array& base_requests) {
            ...
            // устанавливаем флаг готовности базы к работе
        ...
        JsonReader& JsonReader::StatsRequestProcessor(const json::Array& stat_requests) {
            ...
            for (size_t i = 0; i != stat_requests.size(); i++)
            {
                ...
                // создаём и парсим новый запрос на получения информации из базы
        ...
        // сплиттер блоков запросов базового режима
        ...
        // сплиттер блоков запросов базового режима без записи данных роутера
        JsonReader& JsonReader::MakeBaseModeSplitter(const json::Dict& json_request) {
            ...
            // после обработки всех base_requests принудительно запускаем инициализацию роутера 
            _handler.EnableRouterGraphs();  // при данной активации загружаются данные по графам

            // определяем открытие файла для выгрузки сериализированных данных
            if (json_request.count("serialization_settings"))
            {
                // открываем файл для выгрузки базы
                std::ofstream output(FileNameParser(
                    json_request.at("serialization_settings").AsDict()), std::ios::binary);

                // сериализируем данные в поток
        ...
        // сплиттер блоков запросов на получение информации после десериализации данных
        JsonReader& JsonReader::RequestProcessModeSplitter(const json::Dict& json_request) {

            // очищаем текущую очередь запросов
            _current_json_requests.clear();

            // определяем открытие файла для считывания сериализированных данных базы
            if (json_request.count("serialization_settings"))
            {
                // открываем файл для загрузаки базы
                std::ifstream input(FileNameParser(
                    json_request.at("serialization_settings").AsDict()), std::ios::binary);

                // десериализируем данные из потока и заполняем базу
