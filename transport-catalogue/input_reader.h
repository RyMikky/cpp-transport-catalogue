...
namespace transport_catalogue {
	...
	public:
		...
		void AddDistanceProcessing(TransportCatalogue&);

		// выполнение запросов на добавление дистанций по запросу
		void AddDistanceProcessing(TransportCatalogue&, RequestPtr);

		// выполнение запросов на добавление остановки
		void AddStopProcessing(TransportCatalogue&);

		// выполнение запросов на добавление маршрута
		void AddRouteProcessing(TransportCatalogue&);

		// выполнение запросов на получение информации по остановке
		void StopInfoProcessing(TransportCatalogue&, std::ostream&);

		// выполнение запросов на получение информации по маршруту
		void RouteInfoProcessing(TransportCatalogue&, std::ostream&);

		// выполнение запросов на получение информации в общем случае
		void StatInfoProcessing(TransportCatalogue&, std::ostream&);

		// обработчик текущей очереди запросов
		void RequestProcessing(TransportCatalogue&, std::ostream&);
