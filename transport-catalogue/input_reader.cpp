...
namespace transport_catalogue {
	...
	void RequestHandler::AddDistanceProcessing(TransportCatalogue& tc) {
	...
	void RequestHandler::AddDistanceProcessing(TransportCatalogue& tc, RequestPtr request) {
	...
	void RequestHandler::AddStopProcessing(TransportCatalogue& tc) {
	...
	void RequestHandler::AddRouteProcessing(TransportCatalogue& tc) {
	...
	void RequestHandler::StopInfoProcessing(TransportCatalogue& tc, std::ostream& out) {
	...
	void RequestHandler::RouteInfoProcessing(TransportCatalogue& tc, std::ostream& out) {
	...
	void RequestHandler::StatInfoProcessing(TransportCatalogue& tc, std::ostream& out) {
	...
	void RequestHandler::RequestProcessing(TransportCatalogue& tc, std::ostream& out) {

		AddStopProcessing(tc);          // обработка запросов на добавление остановок
		AddDistanceProcessing(tc);      // обработка sub-запросов на добавление дистанций
		AddRouteProcessing(tc);         // обработка запросов на добавление маршрутов
		StatInfoProcessing(tc, out);    // обработка запросов на вывод информации без разделения на типы

		//StopInfoProcessing(tc, out);    // обработка запросов на вывод информации по остановке
		//RouteInfoProcessing(tc, out);   // обработка запросов на вывод информации по маршруту
		...
		void RequestProcessor(TransportCatalogue tc, std::istream& input, std::ostream& output)
		{
			...
			while (InWork)
			{
				try
				{
					...
					request_handler_.RequestProcessing(tc, output);
